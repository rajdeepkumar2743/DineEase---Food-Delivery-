...
public class RestaurantController {
  ...
  public List<Restaurant> SearchRestaurants(@PathVariable("query") String query) {
    ...
    if (ids != null) {
      for (String id : ids) {
        restaurantService.getUser(id).ifPresent(res::add);
      }
    }
    return res;
  }
  ...
  public int updateRestaurantInformation(@RequestBody String jsonInfo)
    ...
    RestaurantInfo newInfo = new RestaurantInfo(open, restaurantName, description, imageUrl, tag1, tag2, tag3);
  ...
  public int deleterRestaurant(@PathVariable("id") String id)
    ...
    if (!orderService.restaurantGetActiveOrders(id).isEmpty()) {
